...
" [1,1]      : Source 1개 ↔ Target 1개 (필수 일대일)
" [0..1,1]   : Source 1개당 Target이 없거나 1개 (옵션 일대일)
" [1..*,1]   : 여러 Source가 Target 1개 참조 (다대일)
" [0..*,1]   : 여러 Source가 Target을 참조하되, Source가 없을 수도 있음 (옵션 포함 다대일)
" [0..1,0..1] : 양쪽 모두 옵션 (서로 없어도 됨)
```
```abap
