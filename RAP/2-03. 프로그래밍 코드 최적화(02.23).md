...
```abap
" [ Target(상대방), Source(나) ] 순서로 작성
" [1, 1]      → Source 1건당 Target이 반드시 1건 매핑 (1:1)
" [0..1, 1]   → Source 1건당 Target이 없거나 최대 1건 (0..1:1)
" [1, 1..*]   → Target 1건을 여러 Source가 참조 (필수 참조 다대일)
" [1, 0..*]   → Target 1건을 여러 Source가 참조 가능 (옵션 참조 다대일)

" 추가: Source가 0..1인 경우 (나도 있을 수도, 없을 수도 있는 관계)
" [1, 0..1]   → Target 1건당 Source가 없거나 최대 1건 매핑
" [0..1, 0..1] → 서로가 서로를 가질 수도, 안 가질 수도 있는 선택적 1:1
...
- `-` (하이픈): 통로 끝에 도달해서 "이 필드를 가져와!"라고 지정하는 구분자
- `\A\_B-Field` → Association을 연속으로 타고 들어가 최종 필드를 조회한다는 의미
- `MANY TO ONE` → 여러 개(MANY) 중에서 하나(ONE)를 골라내는 경로라는 것을 ABAP 컴파일러에게 알려줌
```abap
...
```
```abap
define view entity ZI_FlightInfo
  as select from spfli
  association [ ? ] to sap_planes as _Plane
    on $projection.CarrierId = _Plane.CARRID
{
