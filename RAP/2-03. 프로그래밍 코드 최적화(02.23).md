...
" [1..*,1]   → 여러 Source가 Target 1건 참조 (다대일)
" [0..*,1]   → 여러 Source가 Target 1건 참조 가능 (옵션 포함 다대일)
...
| WHERE 절에서 Association 필드 사용 (예: WHERE _Dept.Name = 'A') | SELECT { } 영역에서 Association 필드 사용 (예: _Dept.Name) | 실제 SQL JOIN 발생 여부 |
|------------------------------------------------------------------|-------------------------------------------------------------|--------------------------|
| X                                                                | X                                                           | No (조인 안 발생)        |
| X                                                                | O                                                           | Yes (출력 위해 JOIN)     |
| O                                                                | X                                                           | Yes (조건 필터 위해 JOIN)|
| O                                                                | O                                                           | Yes (당연히 발생)        |
