...
### 3) CDS에서 CAST로 타입 변환 가능
- CDS는 CAST를 이용해 조회 시점에 데이터 타입을 원하는 형식으로 변환 (DB 데이터 변경X)
...
- CDS에서 2단계 이상 경로 표현식을 사용하면, 중간 association의 키 필드를 projection에 반드시 포함해야 함
- `_Scarr._spfli as _Spfli` 을 통해 Scarr 안의 또 다른 association(Spfli)을 OData에서 확장 조회할 수 있게 projection에 노출
- `@OData.publish: true` (루트가 아니면) 없으면 그 CDS와 그 안의 association 모두 OData로 조회 불가
