...
<img width="534" height="137" alt="image" src="https://github.com/user-attachments/assets/c7d007db-5997-416d-952d-a1322844a529" />
...
- **6) NEW( ) 객체 생성**
- **7) CAST( ) / INSTANCE OF / CASE TYPE OF**
- **8) VALUE( )**
- **9) REF( )**
- **10) CONV( )**
- **11) EXACT( )**
...
- `DATE = USER` → 사용자 로케일 기준 날짜 형식 (예: `2026.02.03`, `02/03/2026` 등)
- `NUMBER = USER` → 사용자 로케일 기준 숫자 형식 (소수점 / 천 단위 구분 자동)
```abap
*************** 기존 방식 ****************
...
*************** 최신 방식 + 날짜 포맷 (user) ****************
gv_string = |Today is { sy-datum DATE = USER }|.

*************** 최신 방식 + 숫자 포맷 (user) ****************
gv_string = |PI value: { gc_pi NUMBER = USER }|.
```
</br>
...
| 구분 | 함수명 | 한줄 설명 | 사용 형식 (Expression) |
|---|---|---|---|
| 비교 | `distance` | 두 문자열 간 편집 거리 계산 | `RESULT = distance( val1 = s1 val2 = s2 ).` |
| 공백 | `condense` | 불필요한 공백 제거 | `RESULT = condense( val = s ).` |
| 결합 | `concat_lines_of` | 내부테이블 여러 줄을 문자열로 결합 | `RESULT = concat_lines_of( table = itab sep = ',' ).` |
| 변환 | `escape` | 특수문자 이스케이프 처리 | `RESULT = escape( val = s format = cl_abap_format=>e_html_text ).` |
| 검색 | `find` | 문자열에서 위치 검색 | `RESULT = find( val = s sub = sub ).` |
| 검색 | `find_end` | 문자열 끝 기준 검색 | `RESULT = find_end( val = s sub = sub ).` |
| 검색 | `find_any_of` | 문자 집합 중 하나라도 포함된 위치 | `RESULT = find_any_of( val = s sub = chars ).` |
| 검색 | `find_any_not_of` | 집합에 없는 문자 위치 | `RESULT = find_any_not_of( val = s sub = chars ).` |
| 삽입 | `insert` | 특정 위치에 문자열 삽입 | `RESULT = insert( val = s sub = sub off = n ).` |
| 반복 | `repeat` | 문자열 반복 | `RESULT = repeat( val = s occ = n ).` |
| 치환 | `replace` | 문자열 치환 | `RESULT = replace( val = s sub = old with = new ).` |
| 변환 | `reverse` | 문자열 뒤집기 | `RESULT = reverse( val = s ).` |
| 추출 | `segment` | 특정 구간 추출 | `RESULT = segment( val = s off = n len = m ).` |
| 정렬 | `shift_left` | 왼쪽 이동 | `RESULT = shift_left( val = s ).` |
| 정렬 | `shift_right` | 오른쪽 이동 | `RESULT = shift_right( val = s ).` |
| 추출 | `substring` | 일부 추출 | `RESULT = substring( val = s off = n len = m ).` |
| 추출 | `substring_from` | 위치부터 끝까지 추출 | `RESULT = substring_from( val = s off = n ).` |
| 추출 | `substring_before` | 특정 문자열 이전 추출 | `RESULT = substring_before( val = s sub = sub ).` |
| 추출 | `substring_after` | 특정 문자열 이후 추출 | `RESULT = substring_after( val = s sub = sub ).` |
| 추출 | `substring_to` | 시작부터 지정 길이까지 추출 | `RESULT = substring_to( val = s len = n ).` |
| 대소문자 | `to_upper` | 대문자 변환 | `RESULT = to_upper( s ).` |
| 대소문자 | `to_lower` | 소문자 변환 | `RESULT = to_lower( s ).` |
| 대소문자 | `to_mixed` | 혼합 대소문자 변환 | `RESULT = to_mixed( s ).` |
| 대소문자 | `from_mixed` | 혼합 대소문자 해제 | `RESULT = from_mixed( s ).` |
| 변환 | `translate` | 문자 일괄 변환 | `RESULT = translate( val = s from = f to = t ).` |
| 비교 | `matches` | 패턴 일치 여부 | `RESULT = matches( val = s regex = r ).` |
| 비교 | `contains` | 문자열 포함 여부 | `RESULT = contains( val = s sub = sub ).` |
</br>
