...
- ALV 표준 툴바에 사용자 정의 버튼을 추가·제어하기 위해 사용되는 이벤트
...
CLASS lcl_event_handler IMPLEMENTATION.
  METHOD on_toolbar.
    ...
    CLEAR ls_button.
    ls_button-function  = '<FUNC_CODE>'.   " 버튼 Function Code (USER_COMMAND에서 구분)
    ls_button-icon      = <ICON>.          " 버튼에 표시되는 아이콘
    ls_button-text      = '<TEXT>'.        " 버튼에 표시되는 텍스트
    ls_button-quickinfo = '<QUICKINFO>'.   " 마우스 오버 시 표시되는 툴팁 설명
    ls_button-butn_type = '0'.             " 버튼 타입 (0: 일반 버튼)
