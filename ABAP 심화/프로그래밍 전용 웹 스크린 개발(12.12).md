...
- 화면 300으로의 예약은 무시되고, 프로그램이 종료되거나 이전 화면으로 복귀
```abap
SET SCREEN 300.
LEAVE TO SCREEN 0.
```
...
- 300번 화면이 계속 떠있어 다음 명령(LEAVE SCREEN)이 실행되지 못하고 갇힘
```abap
CALL SCREEN 300.
LEAVE SCREEN.
```
- **예시 2 : CALL SCREEN + LEAVE TO SCREEN 0**
- 화면 300이 실행된 후, 바로 종료되어 호출 전 이전 화면으로 복귀
```abap
CALL SCREEN 300.
LEAVE TO SCREEN 0.
```
- **예시 3 : 종료 명령어 차이**
- `LEAVE TO SCREEN 0` 은 이전 화면으로 복귀
- `SET SCREEN 0. LEAVE SCREEN.` 은 프로그램을 최종 종료
```abap
" 300번 화면 내부에서 실행 시 동작 비교
LEAVE TO SCREEN 0.
...
### 3) LEAVE SCREEN
- 현재 화면을 종료
- CALL SCREEN으로 열린 화면에서는 이전 화면으로 복귀
- SET SCREEN 후 사용 시 지정된 화면으로 이동
```abap
LEAVE SCREEN.
```
...
LEAVE TO SCREEN 300.
```
- 현재 화면을 종료하고 호출한 이전 화면으로 복귀하거나 프로그램을 종료하는 명령어
```abap
LEAVE TO SCREEN 0.
```
</br>
