...
- **SUBMIT ZPROGB03_CALLED AND RETURN.**
  ...
  - → `call` - 조회조건 화면
  - → `called` - 'No data' 출력
  - → `called` - 뒤로가기하면 조회조건 화면 출력
  - → `call` - 뒤로가기하면 'CALL 프로그램으로 돌아왔어요!' 출력
  - →  `call` - 뒤로가기하면 조회조건 화면으로 돌아옴
  - → 뒤로가기하면 프로그램 종료
...
- **LEAVE TO TRANSACTION 'ZTCODEB03_CALLED' AND SKIP FIRST SCREEN**
