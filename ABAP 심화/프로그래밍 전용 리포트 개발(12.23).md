...
```
```abap
SELECT-OPTIONS so_dept FOR zdept-deptno.
PARAMETERS city_fr TYPE string.
...
AT SELECTION-SCREEN ON so_dept.
  ...
  IF ( so_dept-low  LT '060000' AND so_dept-high LT '060000' )
   ...
   AND city_fr = 'FRANKFURT'.

    MESSAGE e046(bc405).  " 에러 → 화면에 머무름
  ENDIF.
```
</br>
</br>

### 5) Selection Screen 서브스크린 & 탭스트립
- 서브스크린 코드 형식
```abap
SELECTION-SCREEN BEGIN OF SCREEN <서브스크린번호> AS SUBSCREEN.
  PARAMETERS / SELECT-OPTIONS <입력필드>.
SELECTION-SCREEN END OF SCREEN <서브스크린번호>.
```
- 탭스트립 코드 형식
```abap
SELECTION-SCREEN BEGIN OF TABBED BLOCK <탭블록명> FOR <라인수> LINES.

  SELECTION-SCREEN TAB (<탭_텍스트_최대길이>) <탭텍스트변수>
    USER-COMMAND <OKCODE> DEFAULT SCREEN <서브스크린번호>.

SELECTION-SCREEN END OF BLOCK <탭블록명>.
```
- 탭스트립 초기 활성 탭 / 서브스크린 지정
```abap
<탭블록명>-activetab = '<USER-COMMAND>'.
<탭블록명>-dynnr     = '<서브스크린번호>'.
```
```abap
"----------------------------------------
