...
### 4) 추가 Selection Screen 호출 (Calling Additional Selection Screens)
- 기본 Selection Screen(Screen 1000)에서 조건에 따라 다른 Selection Screen(예: 1100) 을 추가로 띄움
```abap
...
- Selection Screen을 서브스크린(Subscreen) 으로 만들어 하나의 메인 화면 안에서 여러 선택 화면을 나눠서 사용
```abap
...
PARAMETERS p_country TYPE spfli-countryfr.
SELECTION-SCREEN END OF SCREEN 103.
```
- Selection Screen에 탭(Tabstrip) 을 만들어 여러 선택 화면을 탭으로 전환해서 표시
- 각 탭은 서브스크린(Screen 번호) 과 연결됨
</br>
</br>
</br>
