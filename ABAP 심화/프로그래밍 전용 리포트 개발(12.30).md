...
- `CFIELDNAME` → 금액 컬럼에 대해 행별로 적용할 통화 필드명을 지정함
- `CURRENCY` → 금액 컬럼 전체에 고정 통화 코드를 적용함
- `QFIELDNAME` → 수량 컬럼에 대해 행별로 적용할 단위 필드명을 지정함
- `QUANTITY` → 수량 컬럼 전체에 고정 단위를 적용함
</br>
</br>
...
*---------------------------------------------------------------------*
CLEAR gs_fcat.
gs_fcat-fieldname = 'CANCELLED_ICON'.   " 취소 여부를 표시할 아이콘 컬럼
gs_fcat-icon      = 'X'.                " 컬럼 값을 아이콘으로 표시
gs_fcat-coltext   = 'Cancelled'.        " 컬럼 헤더 텍스트
APPEND gs_fcat TO gt_fcat.

CLEAR gs_fcat.
gs_fcat-fieldname = 'CANCELLED'.        " 기존 취소 여부 컬럼
gs_fcat-no_out    = 'X'.                " 화면에서 숨김
APPEND gs_fcat TO gt_fcat.

CLEAR gs_fcat.
gs_fcat-fieldname = 'PASSFORM'.         " 여권 형식 컬럼
gs_fcat-no_out    = 'X'.                " 화면에서 숨김
APPEND gs_fcat TO gt_fcat.

CLEAR gs_fcat.
gs_fcat-fieldname = 'PASSBIRTH'.        " 여권 생년월일 컬럼
gs_fcat-no_out    = 'X'.                " 화면에서 숨김
APPEND gs_fcat TO gt_fcat.

CLEAR gs_fcat.
gs_fcat-fieldname = 'PHONE'.            " 고객 전화번호 컬럼
gs_fcat-ref_field = 'TELEPHONE'.        " SCUSTOM-TELEPHONE 참조
gs_fcat-ref_table = 'SCUSTOM'.          " 전화번호 타입 DDIC 참조
gs_fcat-col_pos   = 4.                  " 컬럼 출력 위치 지정
APPEND gs_fcat TO gt_fcat.
