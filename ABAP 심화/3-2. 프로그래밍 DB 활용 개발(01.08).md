...
- Number Range Object는 고객용이므로 이름을 `Z` 또는 `Y` 로 시작해야 함
- Number Length Domain은 Number Range Object 생성 시 필수값
- Number Range의 길이는 도메인에서 `NUMC` 또는 `CHAR` 타입으로 정의하며 최대 20자리까지 가능
