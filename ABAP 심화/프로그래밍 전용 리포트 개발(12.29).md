...
DATA: <gt_table> TYPE TABLE OF <row_type>,
      <gs_row>   TYPE <row_type>.

DATA <gs_colfield> LIKE LINE OF <gs_row>-it_colfields.
```
```abap
TYPE-POOLS col.
...
DATA gs_colfield LIKE LINE OF gs_booking-it_colfields.
```
- 셀 색상 처리 형식
```abap
LOOP AT <gt_table> INTO <gs_row>
  ...
  <gs_colfield>-fname      = '<FIELDNAME>'.
  <gs_colfield>-color-col  = <COLOR>.
  <gs_colfield>-color-int  = '1'.
  <gs_colfield>-color-inv  = '0'.

  APPEND <gs_colfield> TO <gs_row>-it_colfields.
...
LOOP AT gt_bookings INTO gs_booking
  ...
  MODIFY gt_bookings FROM gs_booking
    TRANSPORTING it_colfields.
ENDLOOP.
```
- Layout + ALV 연결 형식
