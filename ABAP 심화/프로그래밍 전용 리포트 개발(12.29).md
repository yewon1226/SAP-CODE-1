...
```
```abap
DATA gs_stable TYPE LVC_S_STBL.    " TOP
...
```
```abap
DATA gs_layout TYPE LVC_S_LAYO.            " TOP

gs_layout-grid_title = 'Booking List'. " Title 적용        " F01
...
CALL METHOD go_alv->set_table_for_first_display            " PBO
  EXPORTING
...
- `SUBTOT = 'X'` : 해당 정렬 필드 기준으로 소계(Subtotal) 라인을 출력. `DO_SUM = 'X'` 가 설정된 합계 대상 필드가 있어야 실제 소계 값이 계산·출력됨.
```abap
...
DATA: BEGIN OF gs_data.
        INCLUDE TYPE sbook.
DATA: excp_fld TYPE char1,
      col_fld  TYPE char4,        " 예: 'C310'
      lt_scol  TYPE lvc_t_scol,   " 셀 색상
     END OF gs_data.
...
**1) `TOP` 모듈 수정**
- `SY-REPID` : 현재 실행 중인 ABAP 프로그램명(Report ID)
```abap
TYPE-POOLS COL.
...
DATA: gs_variant TYPE disvariant,
      ...
      gt_sorter TYPE LVC_T_SORT, " 정렬(itab)
      gs_sorter TYPE LVC_S_SORT, " 정렬(str)
      gt_filter TYPE LVC_T_FILT, " 필터(itab)
      gs_filter TYPE LVC_S_FILT. " 필터(str)
...
FORM set_layout.
  ...
  gs_layout-excp_led = 'X'.
  gs_layout-info_fname = 'COL_FLD'. " 변수명 알려줌
  gs_layout-ctab_fname = 'LT_SCOL'. " 변수명 알려줌
ENDFORM.
...
FORM set_sorter.
  IF gt_sorter IS INITIAL.
    ...
    APPEND gs_sorter TO gt_sorter. " itab 맨 마지막에 추가
