...
### 2. RANK()
- 동일한 값이면 같은 순위,
- 다음 순위는 건너뜀 (1,2,3,3,5 …)
```sql
RANK() OVER (ORDER BY ORDER_DATE ASC)
```

### 3. DENSE_RANK()
- 동일한 값은 같은 순위,
- 다음 순위는 건너뛰지 않음 (1,2,3,3,4 …)
```sql
DENSE_RANK() OVER (ORDER BY ORDER_DATE ASC)
```

### 4. PARTITION BY 사용
- 회원별(MEM_NO 그룹별) 주문일 순위.
- 각 회원의 첫 번째 주문, 두 번째 주문… 을 알 수 있음
```sql
ROW_NUMBER() OVER (PARTITION BY MEM_NO ORDER BY ORDER_DATE ASC)
```
</br>
</br>

### 1. 순위 함수
- SALES 테이블의 주문일자(ORDER_DATE)를 기준으로 순위를 매김
```sql
/***************순위 함수***************/ 
...
### 2. 순위 함수+ PARTITION BY: 그룹별 순위
- 회원(MEM_NO)별로 주문일자 순위를 매김
```sql
...
### 3. 집계 함수(누적)
- ORDER_DATE 순으로 데이터를 정렬하면서, 각 시점까지의 누적 합계/평균/최댓값/최솟값 등을 구함
```sql
/***************집계 함수(누적)***************/ 
...
### 4. 집계 함수(누적)+ PARTITION BY: 그룹별 집계 함수(누적)
- 회원별(MEM_NO) 누적 통계를 구함
```sql
