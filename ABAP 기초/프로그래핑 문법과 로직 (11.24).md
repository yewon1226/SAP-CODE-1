...
- SELECT SINGLE로 일부 필드만 조회하면, 조회한 필드만 값이 들어가고 나머지 필드는 모두 초기화
```abap
...
- 구조 간 동일한 이름을 가진 필드만 선택적으로 복사하는 ABAP 구문
```abap
...
- 한 줄씩 DB에서 읽어오고, 읽을 때마다 바로 처리하는 방식
- 다 건 SELECT 반복 조회회
- `sy-dbcnt` 사용 가능
```abap
...
```
```abap
SELECT-OPTIONS: so_conn FOR spfli-connid DEFAULT '17'.
```
</br>
</br>
</br>
