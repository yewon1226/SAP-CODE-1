...
| 키워드   | VALUE 사용 여부 | Call 방식           | 설명                               | 호출부 변수에 영향                     |
|----------|----------------|------------------|----------------------------------|------------------------------------|
| USING    | VALUE()         | Call by Value       | 값을 복사해서 전달                   | 서브루틴에서 바꿔도 호출부 변수는 변경되지 않음 |
| USING    | 없음            | Call by Reference   | 참조 전달 (주소 전달)               | 서브루틴에서 바꾸면 호출부 변수도 변경됨 |
| CHANGING | VALUE()         | Call by Reference   | 값 전달 + 수정 가능                 | 서브루틴에서 바꾸면 호출부 변수도 변경됨 |
| CHANGING | 없음            | Call by Reference   | 값 전달 + 수정 가능                 | 서브루틴에서 바꾸면 호출부 변수도 변경됨 |
