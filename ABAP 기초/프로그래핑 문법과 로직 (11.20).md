...
**3) 테이블 타입 (Internal Table)**
- Structure 여러 개를 저장하는 리스트 형태
```abap
DATA 내부테이블명 TYPE TABLE OF 구조체타입명.
DATA 작업영역명   TYPE 구조체타입명.
```
```abap
...
- MOVE 하면 필드 위치 기준으로 메모리 그대로 복사되므로, 이름이 달라서 값이 엉뚱한 곳에 들어갈 수 있음.
```abap
