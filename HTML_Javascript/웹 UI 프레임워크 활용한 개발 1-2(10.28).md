...
onInit() {
    ...
    // JSONModel 생성
    let oModelArray = new JSONModel(oDataArray);
    let oModelObject = new JSONModel(oDataObject);

    // View에 모델 연결
    this.getView().setModel(oModelArray, "arrayModel");
    this.getView().setModel(oModelObject, "objectModel");
}
```
</br>
</br>
...
- 배열 데이터 접근
```html
<Page id="page" title="{arrayModel>/name/0/firstName}">
    <!-- arrayModel의 name 배열 첫 번째 객체(인덱스 0번)의 firstName -->
</Page>
```
- 객체 데이터 접근
```html
<Page id="page" title="{objectModel>/firstName}">
    <!-- objectModel의 firstName -->
</Page>
```
- 다중 속성 바인딩
```html
<Page id="page" title="{local>/name/firstName} {local>/name/lastName}">
