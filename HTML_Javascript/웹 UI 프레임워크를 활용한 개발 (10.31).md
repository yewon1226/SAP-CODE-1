# 10/31 기록
### node_modules 삭제 후 다시 실행
- 터미널 경로를 해당 프로젝트 경로로 맞춰주기
- cd 이용
- npm install
- 실행
</br>

### this.byId()
- View에 정의된 컨트롤을 ID로 찾아서 반환
```java
let oTable = this.byId("idCustomers2");
oTable.addItem(new sap.m.ColumnListItem()); // 테이블에 행 추가 가능
```
</br>

### getSelectedItem()
- 리스트나 테이블에서 현재 선택된 항목을 가져오는 메서드
```java
let oRow = oTable.getSelectedItem();
if (!oRow) {
    sap.m.MessageToast.show("선택된 고객이 없습니다.");
    return;
}
```
</br>

### getBindingContext()
- 해당 아이템(oRow)이 바인딩된 모델의 컨텍스트(Context) 를 반환합니다.
- Context는 모델 내에서 이 행이 참조하는 경로(Path) 를 포함하고 있습
```java
let oBindingContext = oRow.getBindingContext("customer");
// 예: sPath = "/Customers/3"
let sPath = oBindingContext.getPath();
```
</br>

### getBinding()
- 특정 UI 컨트롤이 가지고 있는 Aggregation(집합)이 어떤 데이터와 연결되어 있는지 가져오는 함수
```java
getBinding("aggregationName")
``
```java
this.byId("idProductsTable").getBinding("items").filter(aFilter);
this.byId("idProductsTable").getBinding("items").sort(aSorter);
```
</br>

### < 예제 1 >
```java
onReadBooking: function(oEvent) {
      let table = this.byId("idCustomers2");  // this.byId 사용해서 Customers 테이블 가져오기
      let oRow = table.getSelectedItem();     // 현재 선택한 아이템 정보 들어옴   

      let oBindingContext = oRow.getBindingContext("customer");
      let sPath = oBindingContext.getPath();
      let oCustomer = this.getView().getModel("customer").getProperty(sPath);

      let oBookingModel = this.getView().getModel("booking");
      oBookingModel.setProperty("/result", oCustomer._Bookings);
}
```
</br>

### oEvent.getParameter("value") 와 this.byId("idSomething")
- view.xml 파일
```html
<Input id="idInput" placeholder="이름 입력" change=".onInputChange" />
<Button text="이름 읽기 (byId)" press=".onReadInputValue" />
```
- controller.js 파일
```java
onInputChange: function (oEvent) {
    // 👉 방법 ① 이벤트로부터 값 가져오기
    let sValue = oEvent.getParameter("value");
    console.log("이벤트 방식:", sValue);
},

onReadInputValue: function () {
    // 👉 방법 ② 컨트롤 직접 접근해서 값 가져오기
    let sValue = this.byId("idInput").getValue();
    console.log("byId 방식:", sValue);
}
```
</br>

### OData 데이터 구조 개념
- 전역모델
<img width="521" height="322" alt="image" src="https://github.com/user-attachments/assets/9c232c0e-eca7-4dac-9a7b-e30c8d8132f4" />
</br>
</br>
</br>

- EntitySet = 전체 컬렉션
- Entity = 한 건의 데이터
- Complex Type (a) = Entity 안의 객체 필드
- EntityType 속성 = 객체 안의 실제 값
```
EntitySet      → 전체 배열 (전체 데이터 세트)
 └ Entity      → 배열 안의 하나의 레코드
     └ a       → 객체(Complex Type)
         └ Field → EntityType 속성 (실제 값)
```
```java
let entities = this.getView().getModel("model").getProperty("/EntitySet"); // 배열
let entity = entities[0];      // 첫 번째 Entity
let a = entity.a;              // Complex Type
let fieldValue = a.Field;      // 속성 값
```
```java
{
  "EntitySet": [
    {
      "Name": "홍길동",
      "a": {
        "Field": "서울"
      }
    },
    {
      "Name": "박길동",
      "a": {
        "Field": "부산"
      }
    }
  ]
}
```
</br>

### filters
- 테이블이나 리스트의 데이터 중 조건에 맞는 항목만 표시
- 조건은 FilterOperator로 지정
- EQ는 같다, NE는 다르다, GT는 보다 크다, GE는 크거나 같다, LT는 보다 작다, LE는 작거나 같다, BT는 두 값 사이, Contains는 문자열 포함
</br>

- controller.js 파일
```java
new Filter(
    sPath,               // 필드 이름 (예: "ProductName")
    sOperator,           // 연산자 (FilterOperator.EQ, Contains 등)
    vValue1,             // 값1
    vValue2 (옵션)       // 값2 (BT 사용 시)
)
```
```java
let aFilters = [
    new Filter("ProductName", FilterOperator.Contains, "Queso"),
    new Filter({
        path: "UnitsInStock",
        operator: FilterOperator.BT,
        value1: 10,
        value2: 100
    })
];
```
</br>

- view.xml 파일
```html
items="{
    path: '<모델명>><EntitySet>',
    filters: [
        { path: '<필드명>', operator: '<연산자>', value1: '<값>', value2: '<값2(옵션)>' },
        ...
    ]
}"
```
```html
<Table id="idProductsTable" items="{
    path: 'product>/Products',
    filters: [
        { path: 'ProductName', operator: 'Contains', value1: 'Queso' },
        { path: 'UnitsInStock', operator: 'BT', value1: 10, value2: 100 }
    ]
}">
    <columns>
        <Column><Text text="Product Name"/></Column>
        <Column><Text text="Units In Stock"/></Column>
    </columns>
    <items>
        <ColumnListItem>
            <cells>
                <Text text="{product>ProductName}"/>
                <Text text="{product>UnitsInStock}"/>
            </cells>
        </ColumnListItem>
    </items>
</Table>
```
</br>
</br>

### sorter
- 테이블이나 리스트의 데이터를 특정 필드를 기준으로 정렬
- true → 내림차순(descending), false → 오름차순(ascending)
</br>

- controller.js 파일
```java
new Sorter(
    sPath,        // 정렬할 필드 이름 (예: "SupplierID")
    bDescending   // true: 내림차순, false: 오름차순
)
```
```java
let aSorters = [
    new Sorter("SupplierID", true),   // SupplierID 내림차순
    new Sorter("UnitsInStock", false) // UnitsInStock 오름차순
];

// 테이블에 적용
let table = this.byId("idProductsTable");
table.getBinding("items").sort(aSorters);
```
</br>

- view.xml 파일
```html
<Table
    id="idProductsTable"
    items="{
        path: '<모델명>><EntitySet>',
        sorter: [
            { path: '<필드명1>', descending: true/false },
            { path: '<필드명2>', descending: true/false }
        ]
    }"
>
    <columns>
        <Column><Text text="컬럼1"/></Column>
        <Column><Text text="컬럼2"/></Column>
    </columns>

    <items>
        <ColumnListItem>
            <cells>
                <Text text="{<모델명>><필드명1>}"/>
                <Text text="{<모델명>><필드명2>}"/>
            </cells>
        </ColumnListItem>
    </items>
</Table>
```
```html
<Table id="idProductsTable" items="{
    path: 'product>/Products',
    sorter: [
        { path: 'SupplierID', descending: true },
        { path: 'UnitsInStock', descending: false }
    ]
}">
    <columns>
        <Column><Text text="Supplier ID"/></Column>
        <Column><Text text="Units In Stock"/></Column>
    </columns>
    <items>
        <ColumnListItem>
            <cells>
                <Text text="{product>SupplierID}"/>
                <Text text="{product>UnitsInStock}"/>
            </cells>
        </ColumnListItem>
    </items>
</Table>
```
